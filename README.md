...
- 支持为六根弦逐一设置音名与八度，创建、编辑、切换和删除自定义方案。
- 可将自定义方案下载为可共享的 JSON 文件；粘贴 GitHub 文件页、Raw 或 Gist 链接即可导入 JSON 配置。
...
在“调弦设置 → 我的调弦”中选择“导出”，浏览器会下载一个 `.web-tune.json` 文件。将该文件提交到 GitHub 后，使用“从 GitHub 导入”粘贴文件页、Raw 或 Gist 链接即可导入。导入支持一个方案或下列集合格式：
